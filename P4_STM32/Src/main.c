...
int main(void)
{
	...
	Queue_appHandle = osMessageQueueNew (16, sizeof(SIZEOFMESSAGE), &Queue_app_attributes);

	/* creation of Queue_send */
	Queue_sendHandle = osMessageQueueNew (16, sizeof(SIZEOFMESSAGE), &Queue_send_attributes);

	Queue_send_HuartHandle = osMessageQueueNew (16, sizeof(SIZEOFMESSAGE), &Queue_send_Huart_attributes);

	/* creation of Queue_send */
	Queue_read_HuartHandle = osMessageQueueNew (16, sizeof(SIZEOFMESSAGE), &Queue_read_Huart_attributes);
...
void StartTask05(void *argument)
{
	...
	for(;;)
	{
		...
		if(HAL_UART_Receive(&huart3,(uint8_t *) message,5,0xFFFF)== HAL_OK ){
			osMessageQueuePut(QUEUE_READ,(uint8_t *) message,0,10);
		}
